...
import { getUsersMfa } from "../../app/lib/data";
...
const insightsData: Array<{ type: string; insight: string }> = [
  {
    type: "Push",
    insight:
      "Push notifications are the most common MFA method, with 10 threats detected this month.",
  },
  {
    type: "Phone",
    insight:
      "Phone-based MFA methods have also seen 10 threats detected this month.",
  },
  {
    type: "Text",
    insight:
      "Text-based MFA methods are less common, with 5 threats detected this month.",
  },
];

export default function UsersMfa() {
  const [data, setData] = useState<UsersMfaItem[] | null>(null);
  const chartRef = useRef<HTMLCanvasElement>(null);
  const chartInstanceRef = useRef<Chart | null>(null);

  useEffect(() => {
    getUsersMfa().then(setData);
  }, []);
  ...
  useEffect(() => {
    if (data && chartRef.current) {
      ...
      const labels = data.map((item: UsersMfaItem) => item.category);
      const values = data.map((item: UsersMfaItem) => item.threat_count);
          ...
          <div className="w-full h-full flex flex-col lg:flex-row bg-white/5 rounded-lg shadow-lg overflow-hidden divide-x divide-gray-200">
            <div className="px-4 py-5 sm:px-6 flex flex-col justify-center">
              <p className="text-base/7 font-semibold text-white">
                Users Multi-Factor Authentication Overview
              </p>
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                Overview of multi-factor authentication statistics and trends.
              </p>
            </div>
